...
import 'package:duo_zui/tool/CommonUtils.dart';
...
Widget ListBottomWidget(BuildContext context,Post post){
    return Row(
      children: <Widget>[
        DescriptionLabel(context, post.category == null ? '' : post.category.title),
        IconText(context, post.commentCount, 'feedComment.png'),
        IconText(context, post.praiseCount, 'feedPraise.png'),
        Padding(padding: EdgeInsets.all(5),),
        DescriptionLabel(context, post.publishTime),
      ],
    );
}
...
Widget DescriptionLabel(BuildContext context,String text,{double fontSize = 12,int maxLines = 4}){
  return Text(text,
    style:TextStyle(
      fontSize: fontSize,
